import type { Client, Guild, GuildMember, Role, TextBasedChannel } from "discord.js";
...
export function requireGuild(context?: ConversationContext, guildId?: string): Guild {
  ...
  if (context?.guildId && context.guildId !== resolvedId) throw new Error("Serveur Discord hors du secteur courant.");
...
export function ensureInContext(context: ConversationContext | undefined, guildId: string): void {
  if (context?.guildId && context.guildId !== guildId) throw new Error("Ressource hors du serveur courant.");
}

export function ensureChannelInContext(context: ConversationContext | undefined, channelId: string): void {
