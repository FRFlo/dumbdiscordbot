...
const list = toolDefinition({ name: "list_messages", description: "Liste les messages récents d'un salon Discord.", inputSchema: z.object({ channelId: z.string(), limit: z.number().int().min(1).max(100).default(25), before: z.string().optional(), after: z.string().optional() }), outputSchema: z.object({ messages: z.array(output) }) });
const search = toolDefinition({ name: "search_messages", description: "Recherche des messages dans les salons accessibles du serveur courant.", inputSchema: z.object({ query: z.string().min(1).max(100), channelId: z.string().optional(), limit: z.number().int().min(1).max(50).default(25) }), outputSchema: z.object({ messages: z.array(output) }) });
const send = toolDefinition({ name: "send_message", description: "Envoie un message dans un salon Discord après approbation.", inputSchema: z.object({ channelId: z.string(), content: z.string().min(1).max(2_000) }), outputSchema: output });
const reply = toolDefinition({ name: "reply_to_message", description: "Répond à un message Discord après approbation.", inputSchema: z.object({ channelId: z.string(), messageId: z.string(), content: z.string().min(1).max(2_000) }), outputSchema: output });
...
export default [
  get.server(async ({ channelId, messageId }, execution: Exec) => { const messages = await fetchMessage(channelId, execution.context); return messageSummary(await messages.fetch(messageId)); }),
  list.server(async ({ channelId, limit, before, after }, execution: Exec) => { const guild = requireGuild(execution.context); const channel = requireChannel(guild, channelId); if (!("messages" in channel)) throw new Error("Salon non textuel."); const messages = await channel.messages.fetch({ limit: limit ?? 25, before, after }); return { messages: [...messages.values()].map(messageSummary) }; }),
  search.server(async ({ query, channelId, limit }, execution: Exec) => { const guild = requireGuild(execution.context); const max = limit ?? 25; const candidates = channelId ? [requireChannel(guild, channelId)] : [...guild.channels.cache.values()].filter((channel) => channel.isTextBased()); const result = []; for (const channel of candidates) { if (!("messages" in channel)) continue; const messages = await channel.messages.fetch({ limit: Math.min(max, 100) }); result.push(...[...messages.values()].filter((message) => message.content.toLowerCase().includes(query.toLowerCase())).map(messageSummary)); if (result.length >= max) break; } return { messages: result.slice(0, max) }; }),
