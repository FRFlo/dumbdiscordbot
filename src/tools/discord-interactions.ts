...
const react = toolDefinition({
	name: "add_reaction",
	description: "Ajoute une réaction à un message après approbation.",
...
const unreact = toolDefinition({
	name: "remove_reaction",
	description: "Retire la réaction du bot après approbation.",
	inputSchema: z.object({
		channelId: z.string(),
		messageId: z.string(),
		emoji: z.string().min(1).max(100),
...
const reactions = toolDefinition({
	name: "list_reactions",
	description: "Liste les réactions d'un message.",
	inputSchema: z.object({ channelId: z.string(), messageId: z.string() }),
	outputSchema: z.object({
		reactions: z.array(
			z.object({ emoji: z.string(), count: z.number(), users: z.array(z.string()) }),
		),
	}),
});
const poll = toolDefinition({
	name: "create_poll",
	description: "Crée un sondage Discord dans un salon après approbation.",
	inputSchema: z.object({
		channelId: z.string(),
...
export default [
	react.server(async ({ channelId, messageId, emoji }, execution: Exec) => {
	...
	unreact.server(async ({ channelId, messageId, emoji }, execution: Exec) => {
		const channel = requireChannel(requireGuild(execution.context), channelId);
		if (!("messages" in channel)) throw new Error("Salon non textuel.");
		await (
			...
			.get(emoji)
			?.users.remove(requireGuild(execution.context).client.user!.id);
		return { removed: true };
	}),
	reactions.server(async ({ channelId, messageId }, execution: Exec) => {
		const channel = requireChannel(requireGuild(execution.context), channelId);
		if (!("messages" in channel)) throw new Error("Salon non textuel.");
		const message = await channel.messages.fetch(messageId);
		return {
			reactions: await Promise.all(
				[...message.reactions.cache.values()].map(async (reaction) => ({
					emoji: reaction.emoji.name ?? reaction.emoji.id ?? "unknown",
					count: reaction.count,
					users: [...(await reaction.users.fetch()).values()].map((user) => user.id),
				})),
			),
		};
	}),
	poll.server(
		async ({ channelId, question, answers, durationHours, allowMultiselect }, execution: Exec) => {
			const channel = requireChannel(requireGuild(execution.context), channelId);
