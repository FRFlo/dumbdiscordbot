...
const definition = toolDefinition({
  name: "approval",
  description: "Demande une approbation Discord groupée et retourne un jeton limité à plusieurs actions et à leurs cibles précises.",
  inputSchema: z.union([
    z.object({
...
export function createApprovalTool(manager: ApprovalManager) {
  return definition.server(async (input) => {
    const actions: ApprovalAction[] = "actions" in input
      ? input.actions
      : [{ action: input.action, targetIds: input.targetIds }];
