...
const remove = toolDefinition({ name: "delete_scheduled_event", description: "Supprime un événement Discord après approbation.", inputSchema: z.object({ eventId: z.string(), guildId: z.string().optional(), approvalToken: z.string().optional() }), outputSchema: z.object({ deleted: z.boolean(), id: z.string() }) });
...
export default [
  ...
  remove.server(async ({ eventId, guildId, approvalToken }, execution: Exec) => { await approve("delete_scheduled_event", [eventId], approvalToken, `Supprimer l'événement ${eventId}`); const event = await requireGuild(execution.context, guildId).scheduledEvents.fetch(eventId); await event.delete(); return { deleted: true, id: eventId }; }),
