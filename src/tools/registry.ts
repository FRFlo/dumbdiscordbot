...
export interface RegisteredToolInfo { name: string; category: string; tags: readonly string[]; }
...
export class ToolRegistry {
  ...
  public async loadFromDirectory(directory = import.meta.dirname): Promise<void> {
    for (const file of await this.findToolFiles(directory)) {
      ...
      const category = file.includes("discord-") ? file.split("discord-")[1]?.split(".")[0] ?? "discord" : "core";
  ...
  public register(tool: CodeModeTool, category = "core"): void {
    ...
    const tags = destructive ? ["discord", "destructive", "requires-approval"] : ["discord", "sector-scoped"];
