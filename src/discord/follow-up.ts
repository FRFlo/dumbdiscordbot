...
export class FollowUpState {
	...
	public getActive(key: string, now = Date.now()): ConversationState | undefined {
		const row = this.db
			.query("SELECT last_activity FROM conversation_sessions WHERE session_key = ?")
		...
		const history = this.db
			.query(
				"SELECT role, content FROM conversation_messages WHERE session_key = ? ORDER BY id ASC",
			)
	...
	public record(
		...
		this.db.transaction(() => {
			...
			this.db.run("UPDATE conversation_sessions SET last_activity = ? WHERE session_key = ?", [
				now,
				key,
			]);
		})();
